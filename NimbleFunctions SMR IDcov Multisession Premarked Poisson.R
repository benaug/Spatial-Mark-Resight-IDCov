...
IDSampler <- nimbleFunction(
  ...
  run = function() {
    G.true <- model$G.true[g,1:M.both,1:n.cat]
    ...
    G.latent[1:M1,1:n.cat]=G.latent.marked #known ID sample latent status contributions precalculated
    ...
    model$G.latent[g,1:M.both,1:n.cat] <<- G.latent
  ...
  run = function() {
    for(up in 1:z.ups){ #how many updates per iteration?
      ...
      if(updown==0){#subtract
        ...
        n.z.on=length(z.on)
        if(n.z.on>0){ #skip if no unmarked z's are on to turn off. otherwise nimble will crash
