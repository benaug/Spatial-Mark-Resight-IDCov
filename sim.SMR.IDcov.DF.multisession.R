...
sim.SMR.IDcov.DF.multisession <-
  function(N.session=NA,lambda=NA,n.marked=NA,lam0=NA,theta.d=NA,sigma=NA,K=NA,X=X,buff=NA,
    ...
    for(g in 1:N.session){
      data[[g]] <- sim.SMR.IDcov.DF(N=N[g],n.marked=n.marked[g],marktype=marktype,
    ...
    G.marked <- array(NA,dim=c(N.session,n.marked.max,n.cat))
    G.obs <- array(NA,dim=c(N.session,n.samples.max,n.cat))
    IDlist <- vector("list",N.session)
    for(g in 1:N.session){
      G.marked[g,1:n.marked[g],] <- data[[g]]$G.marked
      G.obs[g,1:n.samples[g],] <- data[[g]]$G.obs
    ...
    out <- list(this.j=this.j,this.k=this.k,samp.type=samp.type,ID.marked=ID.marked, #observed data
              ...
              G.marked=G.marked,G.obs=G.obs,IDlist=IDlist,
