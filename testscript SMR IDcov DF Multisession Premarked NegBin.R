...
source("NimbleModel SMR IDcov DF Multisession Premarked NegBin.R")
source("NimbleFunctions SMR IDcov Multisession Premarked NegBin.R") #no extra DF stuff here
...
theta.d=rep(0.05,N.session) # overdispersion parameter. Assuming shared across df groups here.
...
inits=list(lam0=lam0,sigma=sigma,theta.d=theta.d) #initializing with 1 parameter per session, just set all to same value. In practice, don't init to truth.

#This function structures the simulated data to fit the model in Nimble (some more restructing below)
#Also checks some inits
nimbuild=init.SMR.IDcov.DF.multisession(data,inits,M1=M1,M2=M2,marktype=marktype,obstype="negbin")
...
Niminits <- list(N=N.init,N.UM=N.UM.init,
                 ...
                 lam0.fixed=inits$lam0[1,],sigma.fixed=inits$sigma[1,],theta.d.fixed=0.1,D=0.5)
