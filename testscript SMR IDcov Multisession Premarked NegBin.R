...
source("NimbleModel SMR IDcov Multisession Premarked NegBin.R")
source("NimbleFunctions SMR IDcov Multisession Premarked NegBin.R")
source("init.SMR.IDcov.multisession.R")
...
theta.d=rep(0.05,N.session) # overdispersion parameter
...
data=sim.SMR.IDcov.multisession(N.session=N.session,lambda=lambda,n.marked=n.marked,marktype=marktype,
...
inits=list(lam0=lam0,sigma=sigma,theta.d=theta.d) #initializing with 1 parameter per session, just set all to same value

#This function structures the simulated data to fit the model in Nimble (some more restructing below)
#Also checks some inits
nimbuild=init.SMR.IDcov.multisession(data,inits,M1=M1,M2=M2,marktype=marktype,obstype="negbin")
...
Niminits <- list(N=N.init,N.UM=N.UM.init,
                 ...
                 lam0.fixed=0.75,sigma.fixed=0.75,theta.d.fixed=0.5,D=0.5)
...
parameters=c('D','lambda','lam0.fixed','sigma.fixed','theta.d.fixed','theta.marked','theta.unmarked','gammaMat',
              'n.M','n.UM','N.UM','N')
...
for(g in 1:N.session){
  ...
  p.nodes <- Rmodel$expandNodeNames(paste("p[",g,",","1:",M.both[g],",1:",J[g],"]"))
  ...
  calcNodes <- c(N.node,N.UM.node,y.nodes,lam.nodes,p.nodes)
  ...
  conf$addSampler(target = c("N"),
                  type = 'zSampler',control = list(z.ups=z.ups[g],J=J[g],M1=M1[g],M.both=M.both[g],g=g,
                                                   ...
                                                   N.node=N.node,z.nodes=z.nodes,calcNodes=calcNodes),
                  silent = TRUE)
}
